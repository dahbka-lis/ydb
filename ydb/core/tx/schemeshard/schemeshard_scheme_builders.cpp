    ...
    TString& error)
{
    ...
    const auto& sourceDescription = pathDescription.GetTable();
    const bool hasGeneratedColumns = AnyOf(
        sourceDescription.GetColumns(),
        [](const auto& column) { return column.HasDefaultFromExpression(); });
    if (!hasGeneratedColumns) {
        return true;
    }

    if (sourceDescription.SequencesSize() != 0) {
        error = "Cannot backup table with both generated and serial columns";
        return false;
    }

    auto description = sourceDescription;
    description.ClearCdcStreams();
    for (auto& indexDescription : *description.MutableTableIndexes()) {
        indexDescription.ClearIndexImplTableDescriptions();
    }
