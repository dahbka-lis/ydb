...
THolder<TEvSchemeShard::TEvModifySchemeTransaction> BackupPropose(
    ...
    if (item.ParentIdx == Max<ui32>()) {
        ...
        if (!FillTableDescription(
                ss,
                task,
                TPath::Init(item.SourcePathId, ss),
                exportPath.Child(ToString(itemIdx)),
                error)) {
            return nullptr;
        }
    } else {
